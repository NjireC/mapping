...
$sql = "SELECT
        ...
        ,ss2.id as id ,$col ,CONCAT(auditsen,':',auditfr) AS keyid
         FROM
        projectregsite
        ...
        LEFT JOIN (
          ...
          ) AS audit ON (ss2.sen = audit.auditsen and ss2.fr = audit.auditfr and audit.auditformtype = 2)
            $where ORDER BY ss2.sen ASC,ss2.fr ASC limit $offset, $limit ";
...
$sql2 = "SELECT
        ...
        INNER JOIN ".$prefixtable."ss2 as ss2 
        ON (ss2.edssid = $edssid)
        LEFT JOIN (
            SELECT * FROM $audit_trail as audit_trail 
             GROUP BY CONCAT(auditsen,':',auditfr)
          ) AS audit ON (ss2.sen = audit.auditsen and ss2.fr = audit.auditfr and audit.auditformtype = 2)  
        ORDER BY ss2.sen ASC,ss2.fr ASC";
